...
class Utilities: NSObject {
    ...
    class  func setRightViewIcon(icon: UIImage,field:UITextField) {
        ...
        let width = 15
        let view = UIView(frame: CGRect(x: 8, y:0, width: 45, height: h))
        
        let imageview = UIImageView(frame: CGRect(x: 10, y:Int(Int(h/2 ) - Int(height/2)), width: width, height: height))
